...
function handleTextSelection() {
    ...
    if (selectedText && selectedText.length > 0) {
        ...
        elements.addCommentFab.style.top = `${rect.bottom + window.scrollY + 10}px`;
        elements.addCommentFab.style.left = `${rect.left + window.scrollX}px`;
...
function generateExportText() {
    ...
    output += `  FEEDBACK DE RESPUESTA IA\n`;
    output += `  Generado: ${now}\n`;
    output += `${separator}\n\n`;
    ...
    output += `${separator}\n`;
    output += `  Total de comentarios: ${state.comments.length}\n`;
    output += `${separator}`;
